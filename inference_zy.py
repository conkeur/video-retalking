...
def main():    
    ...
    print("step_load_cost_time: ", time.time() - start_time_load)
    ...
    restorer = GFPGANer(model_path='checkpoints/GFPGANv1.3.pth', upscale=1, arch='clean', \
                        channel_multiplier=2, bg_upsampler=None) # 用于超分
    ...
    base_name = args.face.split('/')[-1] # 取人脸视频的名称
    if os.path.isfile(args.face) and args.face.split('.')[1] in ['jpg', 'png', 'jpeg']:
        args.static = True
    if not os.path.isfile(args.face):
        raise ValueError('--face argument must be a valid path to video/image file')
    elif args.face.split('.')[1] in ['jpg', 'png', 'jpeg']:
        full_frames = [cv2.imread(args.face)] # 读取图像
        fps = args.fps
    else:
        ...
        while True:
            ...
            full_frames.append(frame)  # 将裁剪后的视频帧存储
    

    print ("[Step 0] Number of frames available for inference: "+str(len(full_frames))) # 打印视频帧的数量
    
    # end_time0 = time.time()
    # cost_time0 = end_time0 - start_time0
    print("step0_cost_time: ", time.time() - start_time0)
    
    # face detection & cropping, cropping the first frame as the style of FFHQ
    start_time1 = time.time()
    croper = Croper('checkpoints/shape_predictor_68_face_landmarks.dat') # 预测人脸关键点
    ...
    print("step1_cost_time: ", time.time() - start_time1)
    ...
    print("step2_cost_time: ", time.time() - start_time2)
    # load DNet, model(LNet and ENet)
    # start_time_load = time.time()
    # D_Net, model = load_model(args, device)
    # print("step_load_cost_time: ", time.time() - start_time_load)
    ...
    if not os.path.isfile('temp/'+base_name+'_stablized.npy') or args.re_preprocess:
        ...
        for idx in tqdm(range(len(frames_pil)), desc="[Step 3] Stablize the expression In Video:"):
            ...
            with torch.no_grad():
                output = D_Net(source_img, coeff) # 为每一帧生成新表情
    ...
    print("step3_cost_time: ", time.time() - start_time3)
    ...
    mel = audio.melspectrogram(wav) # 转换为mel谱
    ...
    print("step4_cost_time: ", time.time() - start_time4)
    ...
    imgs_enhanced = []
    for idx in tqdm(range(len(imgs)), desc='[Step 5] Reference Enhancement'): # 这里的imgs是经过表情编辑网络生成的
        img = imgs[idx]
        pred, _, _ = enhancer.process(img, img, face_enhance=False, possion_blending=False) # 关掉人脸增强
        imgs_enhanced.append(pred)
    
    gen = datagen(imgs_enhanced.copy(), mel_chunks, full_frames, None, (oy1,oy2,ox1,ox2))
    
    # gen = datagen(img.copy(), mel_chunks, full_frames, None, (oy1,oy2,ox1,ox2))
    ...
    if args.up_face != 'original':
        ...
        instance.initialize()
        instance.setup()
    print("step5_cost_time: ", time.time() - start_time5)
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(tqdm(gen, desc='[Step 6] Lip Synthesis:', total=int(np.ceil(float(len(mel_chunks)) / args.LNet_batch_size)))):
        ...
        with torch.no_grad():
            ...
            pred, low_res = model(mel_batch, img_batch, reference) # 进行唇形合成
        ...
        for p, f, xf, c in zip(pred, frames, f_frames, coords):
            ...
            # month region enhancement by GFPGAN # 关闭嘴部区域增强
            ...
            # print("post_processing_cost_time: ", time.time() - post_processing_time)
            # pp, orig_faces, enhanced_faces = enhancer.process(pp, xf, bbox=c, face_enhance=False, possion_blending=True)
            # pp, orig_faces, enhanced_faces = enhancer.process(ff, xf, bbox=c, face_enhance=False, possion_blending=True)
            
            # 进行嘴部融合处理
            pp, orig_faces, enhanced_faces = enhancer.process(ff, xf, bbox=c, face_enhance=False, possion_blending=True)
            out.write(pp)
            
            # 不进行嘴部融合，可以提高速度，但嘴部会有较明显粘贴痕迹
            # out.write(ff)
    # print("post_processing_cost_time: ", time.time() - post_processing_time)
    out.release()
    print("step6_cost_time: ", time.time() - start_time6)
    ...
    end_time = time.time()
    cost_time = end_time - start_time_load
...
def datagen(frames, mels, full_frames, frames_pil, cox):
    ...
    # original frames
    check_lamdmarks_time = time.time()
    kp_extractor = KeypointExtractor()
    fr_pil = [Image.fromarray(frame) for frame in frames] # 增强后的表情帧
    lms = kp_extractor.extract_keypoint(fr_pil, 'temp/'+base_name+'x12_landmarks.txt')
    # # # 改
    # lms = np.loadtxt('temp/'+base_name+'x12_landmarks.txt').astype(np.float32)
    # lms = lms.reshape([len(frames), -1, 2])
    frames_pil = [ (lm, frame) for frame,lm in zip(fr_pil, lms)] # frames is the croped version of modified face
    crops, orig_images, quads  = crop_faces(image_size, frames_pil, scale=1.0, use_fa=True)
    inverse_transforms = [calc_alignment_coefficients(quad + 0.5, [[0, 0], [0, image_size], [image_size, image_size], [image_size, 0]]) for quad in quads]
    del kp_extractor.detector
    print("check_lamdmarks_cost_time: ", time.time() - check_lamdmarks_time)

    face_detect_time = time.time()
    oy1,oy2,ox1,ox2 = cox
    face_det_results = face_detect(full_frames, args, jaw_correction=True)
    print("face_detect_cost_time: ", time.time() - face_detect_time)
