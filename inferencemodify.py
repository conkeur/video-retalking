...
# ], stdin=subprocess.PIPE)
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(tqdm(gen, desc='[Step 6] Lip Synthesis:', total=int(np.ceil(float(len(mel_chunks)) / args.LNet_batch_size)))):
        ...
        for p, f, xf, c in zip(pred, frames, f_frames, coords):
            ...
            orig_height,orig_width=ff.shape[:2]
            ...
            ffcrop=(ff[y1:y2, x1:x2])
            restored_img_crop=restored_img[y1:y2, x1:x2]
            mouse_mask_crop=mouse_mask [y1:y2, x1:x2]     
            height, width = ffcrop.shape[:2]
            ...
            if longer_side>4096:
                pyrinputsize=4096
            # print('longerside:',longer_side)
            # print('ffcropsize',ffcrop.shape[:2])
            # restored_img, ff, full_mask = [cv2.resize(x, (pyrinputsize, pyrinputsize)) for x in (restored_img, ff, np.float32(mouse_mask))]
            restored_img_crop, ffcrop, mouse_mask_crop = [cv2.resize(x, (pyrinputsize, pyrinputsize)) for x in (restored_img_crop, ffcrop, np.float32(mouse_mask_crop))]
            # print('ffcropsize:',ffcrop.shape[:2])
            # print('restored_img_crop:',restored_img_crop.shape[:2])
            # print('mouse_mask_crop:',mouse_mask_crop.shape[:2])
            # print('inputsize',pyrinputsize)
            # print('pynum',int(np.log2(pyrinputsize))+1)
            img = Laplacian_Pyramid_Blending_with_mask(restored_img_crop, ffcrop, mouse_mask_crop[:, :, 0], int(np.log2(pyrinputsize))+1)

            # img = Laplacian_Pyramid_Blending_with_mask(restored_img, ff, full_mask[:, :, 0], 10)
            pp = np.uint8(cv2.resize(np.clip(img, 0 ,255), (width, height)))
            ff[y1:y2, x1:x2]=pp 
            pp, orig_faces, enhanced_faces = enhancer.customprocess(ff, xf, bbox=c, face_enhance=False, possion_blending=True)
            # ffprocess.stdin.write(pp.tobytes())#useffprocess to write
            out.write(pp)
    out.release()
    # ffprocess.stdin.close()#close ffprocess
    # ffprocess.wait()#part of ffprocess
